...
- [ ] Backup existing database
  ```bash
  mysqldump -u root -p mooncraft_db > backup_$(date +%Y%m%d).sql
  ```
...
```bash
mysql -u root -p mooncraft_db < DATABASE_MIGRATION_PRODUCTS.sql
```

**For Existing Installations:**
```bash
# Run the migration script
mysql -u root -p mooncraft_db < DATABASE_MIGRATION_IMAGE_FIX.sql

# Or manually execute:
mysql -u root -p mooncraft_db -e "ALTER TABLE products MODIFY COLUMN image LONGTEXT NULL;"
mysql -u root -p mooncraft_db -e "ALTER TABLE products ADD COLUMN IF NOT EXISTS image_data LONGBLOB NULL;"
```
...
```bash
# Stop backend first
mysql -u root -p mooncraft_db < backup_20260707.sql
```
