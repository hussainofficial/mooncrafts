# LUXORA Jewelry - Testing Guide

## Server Details
- **Dev Server**: http://localhost:4201
- **Build Output**: c:\Users\hussa\luxora-jewelry\dist\luxora-jewelry
...
**Expected Result**: 
- User created in localStorage (luxora_users)
...
5. Product is saved to localStorage (luxora_cart)
...
- `luxora_users` - All registered user profiles
- `luxora_auth` - Current logged-in user session
- `luxora_cart` - Shopping cart items
- `luxora_wishlist` - Wishlist items (per user)
...
# Output location
dist/luxora-jewelry/   # Production build output
```
